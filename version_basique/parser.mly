...
%token <string> CHAINE
(*INSTRUCTIONS*)
%token IMPRIMER ENTREE NL FIN SI ALORS 
%token VAVERS
%token <string> REM
...
%type <Syntax.expr_sortie list> expr_list
...
instruction:
| IMPRIMER el=expr_list {IMPRIMER(el)}
...
| REM {REM($1)}
| NL {NL}
| v=VAR EG ex=expr {AFF(v, ex)}

expr_list:
| e=expr_sortie VIRG el=expr_list {e::el}
| e=expr_sortie {[e]}
